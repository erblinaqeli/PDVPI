...
<html>
   ...
   {	
   	$result = mysqli_query($conn7,"CALL SelektoStudEmail('$login_user')");
   ...
   while($res = mysqli_fetch_array($result))
   {
   	...
   	if(empty($password_n) || empty($password_n1) || empty($password_c)) {	
   		if(empty($password_n)) {echo "<div style='text-align:center; border:red; border-radius:10px; color:white; margin:auto; height:1.7em; width:20em; background-color:#09C32E;'>Fusha e fjalëkalimit të ri është e zbrazët!</font><br/></div>";}
   		if(empty($password_n1)) {echo "<div style='text-align:center; border:red; border-radius:10px; color:white; margin:auto; height:1.7em; width:20em; background-color:#09C32E;'>Fusha e fjalëkalimit të ri të përseritur është e zbrazët!</font><br/></div>";}
   		if(empty($password_c)) {echo "<div style='text-align:center; border:red; border-radius:10px; color:white; margin:auto; height:1.7em; width:20em; background-color:#09C32E;'>Fusha e fjalëkalimit të vjetër eshte e zbrazët!</font><br/></div>";}
   	} else {	
   	if($password == $password_c){
   
   		if($password_n == $password_n1){
   		
   		$result = mysqli_query($conn42,"CALL `UpdateUsersP`($uid,'$password_n');");
...
<?php
   ...
   // SELECT `uid_Student`,`username`,`mbiemri`,email,`password`,`ID_usersRole` FROM users WHERE email = '$login_user'
   $result = mysqli_query($conn39,"CALL SelektoStudEmail('$login_user')");
