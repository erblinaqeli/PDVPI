	...
	<section id="sidebar" class="menu" >
      <svg class='menu-logo' onclick="togglemenu()" fill="#fefefe" xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 24 24" width="24px" height="24px"><path d="M 3 5 A 1.0001 1.0001 0 1 0 3 7 L 21 7 A 1.0001 1.0001 0 1 0 21 5 L 3 5 z M 3 11 A 1.0001 1.0001 0 1 0 3 13 L 21 13 A 1.0001 1.0001 0 1 0 21 11 L 3 11 z M 3 17 A 1.0001 1.0001 0 1 0 3 19 L 21 19 A 1.0001 1.0001 0 1 0 21 17 L 3 17 z"/></svg>
			...
			<section id="MenaxhimiPerdoruesve" style="overflow-y: auto;  padding: 5%; ">
					...
					<h2 style="margin-bottom:5%"class="title">IDETË E PROJEKTEVE</h2>
					<?php 
						
						$result2 = mysqli_query($conn38, "CALL SelectShfaqIdeteEProjektit($id)"); //na nevoitet ne query
 ...
 <div class="scroll">
					...
					<table class="alt" style="width: 20%; margin: auto;">
                     ...
                     <th name="KomentetEIdeve" id="KomentetEIdeve">
                        <div>Koment lidhur me Idene e Projektit</div>
                     </th>
			...
			<?php
                    ...
                    $result = mysqli_query($conn26,"CALL SelektoIdentEProjekteve($id)" );//na nevoitet ne query
                    ...
                    while($row = mysqli_fetch_array($result)) {
                        ...
                        if($stud == $login_user)
                        {
                           ...
                           echo"<td><mark>{$row['username']}</mark></td>";
                           echo"<td><mark>{$row['mbiemri']}</mark></td>";
                           echo"<td>{$row['TitulliProjektit']}</td>";
                     ?>
                     <td ><textarea type="text"  name="Koment" style="width: 10em; height:1.7em;" readonly><?php echo $row['Koment'] ?></textarea></td>
                     
                     <?php
                           echo"<td >{$row['KomentetEIdeve']}</td>";
                      
                     ?>
                     <td ><textarea type="text"  name="KomentProf" style="width: 10em; height:1.7em;" readonly><?php echo $row['KomentProf'] ?></textarea></td>
