...
if($result==null)
  mysqli_free_rezultati($result);
			...
			<h2 style="margin-bottom:5%;"class="title">MENAXHO EVIDENCËN E STUDENTËVE</h2>
					...
					<?php
								...
								if (isset($_GET['submit'])) {
									...
									$result = mysqli_query($conn42,"CALL SelectFormulaFE()");
								...
								$uid_Evidenca = $_GET['uid_Evidenca'];
								...
								// WHERE uid_Evidenca=$uid_Evidenca
								$query = mysqli_query($conn43,"CALL UpdateEvidencaL($Java1,$Java2,$Java3,$Java4,$Java5,$Java6,$Java7,$Java8,$Java9,$Java10,$Java11,$Java12,$Java13,$Java14,$Java15,$uid_Evidenca)");
								$query = mysqli_query($conn44,"CALL UpdatePiketEvidenca()");
									$query = mysqli_query($conn45,"UPDATE
									evidencal
									SET
									TotalEL =
										Piket $FE");
 ...
 <div class="scroll">
										...
										<form action="lenda.php?id=<?php echo $id ?>" method="post" style="width: 90%; "> 
                        ...
                        echo"<th>Modifiko</th>";
								...
								while ($row1 = mysqli_fetch_array($query1)) {
									echo"	<form class='form' method='get' action='lenda.php?id=$id'>";

									echo"<input class='input' type='hidden' name='uid_Evidenca' value='{$row1['uid_Evidenca']}' />";
	...
	echo "</form>";
								...
								$query1 = mysqli_query($conn46,"CALL SElectEvidencenLP($id)");
								while ($row1 = mysqli_fetch_array($query1)) {
...
echo "</form>";
					...
					<?php 				
	echo"<form method='post' action='import_EvidencenL.php?id=$id' enctype='multipart/form-data'>";
	...
	echo"</form>"; 
  ?>
					<?php 				echo"<a class='button primary' href=\"export_excelEvidencenL.php?id=$id\">Export Listën E Evidencës</a>"; ?>
